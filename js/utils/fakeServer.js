...
var server = {
     ...
     * @param {?data} data
     */
    login(username, password, callback, data) {
        users = data;
        const userExists = this.doesUserExist(username, password);
    ...
    doesUserExist(username, password) {
        var user = users.results.filter(function(item){if(item.name==username && item.birth_year == password) return item;});
        console.log(user);
        return !(!user.length);
    }
}
